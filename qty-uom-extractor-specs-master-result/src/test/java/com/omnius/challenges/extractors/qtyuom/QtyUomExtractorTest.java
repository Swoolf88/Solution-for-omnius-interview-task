    ...
    public void computeAccuracy_checkIsHigherThan20Percent() throws IOException{
    	...
    	QtyUomExtractor matcher = new LeftMostUOMExtractor();
		    ...
		    while ((line = br.readLine()) != null) {
		      ...
		      String qtyText = values[1];
		      qtyText = qtyText.replace("\"", "");
		      String uomText = values[2];
		      uomText = uomText.replace("\"", "");
		      //System.out.println(lineText + " v1 "+ qtyText +" v2 " + uomText); //print lines
		      total++;		      
		      Pair<String, String> result = matcher.extract(lineText);
		        String qty = result.getFirst();
		        String uom = result.getSecond();
		        if(qty.equals(qtyText) && uom.equals(uomText)){
		        	count=count+1;
		        	assertEquals(qtyText, qty);
			        assertEquals(uomText, uom);
		        }
		        accuracy = (count/total)*100;		        
		    }
		    br.close(); 
			Assert.assertTrue(accuracy > 20);
		    System.out.println("accuracy: " + accuracy); //print accuracy
    }
}
